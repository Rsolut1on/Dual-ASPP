...
class connect_loss(nn.Module):
    def __init__(self,hori_translation,verti_translation,density=None,bin_wide=None, num_class=1):
    ...
    def multi_class_forward(self, c_map, target):
        ...
        onehotmask = F.one_hot(target.long(),self.num_class) # change it to your class number if needed
        onehotmask = onehotmask.permute(0,3,1,2) 
        onehotmask = onehotmask.float()

        ### get your connectivity_mask ###
        con_target = connectivity_matrix(onehotmask,self.num_class) #(B, 8*C, H, W)
        ...
        ### get edges gt###
        class_conn = con_target.view([c_map.shape[0],self.num_class, 8,c_map.shape[2],c_map.shape[3]])
        ...
        class_pred = c_map.view([c_map.shape[0],self.num_class, 8, c_map.shape[2],c_map.shape[3]])
    ...
    def single_class_forward(self, c_map, target):
        ...
        ### get your connectivity_mask ###
        con_target = connectivity_matrix(target,self.num_class)#(B, 8, H, W)
