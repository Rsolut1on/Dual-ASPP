...
if __name__ == '__main__':
    ...
    trans = transforms.Compose([
        ...
        # transforms.mask2gray(),
        # transforms.RandomCrop(size=224, pad_if_needed=True),
        # transforms.RandomResizedCrop(size=224, scale=(.8,.8), ratio=(1.0, 1.0)),
    ...
    for epoch in range(cfgs['max_iter']):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(dataloader):
            ...
            loss = criterion(prediction.softmax(dim=1), labels)
            loss.backward()
            optimizer.step()
            ...
            if i % print_epoch == 0:
                ...
                print(nTP, acc)
                if best_acc < acc:
                    torch.save(net.state_dict(), './checkpoint/' + cfgs['dataset']['testis']['pth']+cfgs['model_name']+'_best.pth')
                    best_acc = acc
        ...
        running_loss = 0.0
        save_history('./checkpoint/' + cfgs['dataset']['testis']['pth']+cfgs['model_name']+'_log.csv', history)
    torch.save(net.state_dict(), './checkpoint/' + cfgs['dataset']['testis']['pth']+cfgs['model_name']+'.pth')
    print('Saved at: ', './checkpoint/' + cfgs['dataset']['testis']['pth']+cfgs['model_name']+'.pth')
